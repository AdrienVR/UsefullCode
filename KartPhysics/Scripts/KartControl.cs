...
public class KartControl : MonoBehaviour {
    ...
    private Vector3 m_gravityResult;
    ...
    private void UpdateCollision()
    {
        ...
        m_newRot = m_transform.rotation * Quaternion.Euler(0, m_angularVelocity * m_dt, 0);
    ...
    private void UpdateGravity()
    {
        ...
        m_gravityResult = m_gravityVector - m_groundNormal * m_gravity;

        m_WheelFL.text = delta.ToString("N2");

        // apply gravity
        if (m_groundDist == m_maxGroundDist || (delta > m_gravityThreshold && m_groundDist > m_maxGravityMove))
        {
            if (m_touchGround)
            {
                m_touchGround = false;
    ...
    RaycastHit CenterRaycast(TextMesh _text)
    {
        ...
        bool hitBool = Physics.Raycast(m_position, dir, out hit, dist, (int)m_groundMask);
        ...
        else
        {
            ...
            m_groundDir = dir * dist;
            m_groundDist = dist;
            m_groundPos = m_position + m_groundDir;
        }
